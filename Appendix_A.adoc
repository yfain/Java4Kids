...
Note the first statement indicating that I work with the code branch called _master_. At this point I have no any other branches. The untracked files are displayed in red, which means that they are either new to Git or have been modified after the last commit to the repository. The changes made to the untracked files are not stored in the Git database.
...
What if Mary wants to add a new feature to the application, but she's afraid that her new code may introduce bugs in the working code? Joe works on adding another feature and he also wants to keep his code separately. There is a solution to this - both Mary and Joe can create their own branches to keep the new code being developed separately from the _master_ branch, which has tested and working code. 

Let's follow how Mary will be adding a new method `surface` to the class `Fish`, which currently looks like this:
...
Now she adds the following method to the class `Fish`:
...
Since Mary was the only programmer who was changing the file _Fish.java_ Git just kept track of all insertions, deletions, changes and re-applied them to the older version of the file during merge. But when more than one programmer works on the same file they could modify the same piece if code in different branches. This may cause merging conflicts and Git will mark the code fragments that need to be merged manually.
...
I'll commit the changes to the _master_ branch, then switch to _mary_ and add the question marks there in the same place where exclamations were added in _master_. So in the branch _mary_ this line will look like this:
...
If you'll open the file _Fish.java_
...
https://github.com/[GitHub] is a Web-base Git repository hosting service, where you can create a shared code repository free of charge as long as you don't mind having your code available for public. People or organizations that want to create private repositories on GitHub can purchase a paid account. But we have nothing to hide, and programmers who have their code published on GitHub are treated with additional respect.
