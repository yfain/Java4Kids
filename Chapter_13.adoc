...
In chapters 6, 7, and 10 you’ve learned how to create simple GUI with JavaFX using such components as buttons, labels and text fields.  This chapter is also about working with different components of JavaFX - you'll be drawing and animating shapes. 
...
JavaFX supports various types of transformations that change coordinates of the components on the scene. Working with coordinates is an advanced topic, but if you'd like to research it on your own, start with reading the http://docs.oracle.com/javase/8/javafx/api/javafx/scene/Node.html[documentation] about the `Node` class. To keep things simple, let's just modify the attributes of the tag `<Circle>` so it'll look like this:
...
We're ready to serve the ball now.


The class `javafx.animation.Timeline` allows you to change the values of properties over a time interval.   
