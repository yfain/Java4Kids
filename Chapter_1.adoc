...
People talk to each other using different languages. Programmers write computer programs like games, calculators, text editors using different programming languages too. Without programs,the screen of your computer, smartphone or a game console would always be black. All these devices are often called _hardware_, and programs are known as _software_. For many many years Java remains one of the most popular computer languages. Just check out this http://www.tiobe.com/content/paperinfo/tpci/index.html[popularity index] by Tiobe. I'm not sure when are you looking at this chart, but I can bet that Java is among the top three languages there. What makes the Java language so hot?
