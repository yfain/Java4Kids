...
This code creates an instance of the animation class `FadeTransition` that will span 1000 milliseconds (1 second) and applies if to the winning button. Then the code sets the fading parameters to change the opacity of the button from 1.0 to 0.1 (changing to 0.0 would make the button completely transparent).

By setting the style count to 10, repeats the animation 10 times. Because of `setAutoReverse(true)` , the animation will proceed forward on the first cycle, then reverses on the second cycle, and so on. The method `play` starts the animation. Adding the following three lines to the method `highlightWinningCombo` will make the winning buttons animate.
...
The screen shot on Figure 10-12 should give you an idea of how the end of the animation cycle will look like.  

[[FIG10-13]]
.Faded winning buttons
image::images/fig_10_13.png[]

Compare this image with Figure 10-11, or better run the code that comes with the book to see it in action.
