...
* The method `say()` will not be printing any messages, but will the prepare a message based on the word or a phrase passed to this method as an argument. This method will build a phrase using the argument and will return it back to the calling program. 
...
public class PetMaster {
    ...
    public static void main(String[] args) {
        ...
        petReaction = myPet.say("Tweet!! Tweet!!"); <4>
...
As you know, fish do not speak (at least they do not do it aloud). But our class `Fish` has been inherited from the class `Pet` that has the method `say()`. This means that nothing stops you from writing `myFish.say();`.

Do you really want our fish to talk? If you do not want this to happen, the class `Fish` has to _override_ the `Pet`’s typical behavior - the method `say()`. If you declare  a method with exactly the same signature in a subclass as in its superclass, the subclass' method will be used instead of  the method of the superclass. Let’s add the following method `say()` to the class `Fish`.
