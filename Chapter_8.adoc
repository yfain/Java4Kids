...
We want the method `signInClicked` to be called on the controller if the user clicks on the button. For this to happen, we can specify the name of this method as a handler of the `onAction` event in the Button element in `signin.fxml` as shown on Figure 8-2.
